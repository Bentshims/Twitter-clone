...
export default class TweetsController {
  ...
  public async profil({ view, params, auth }: HttpContext) {
    ...
    const isFollow = await authUser.related('following').query().where('users.id', user.id).first()

    //les notifications
    const newSNotification = await authUser
    ...
    if (user.id === authUser.id) {
      ...
      return view.render('pages/profil-tweet', {
        user,
        followers,
        followings,
